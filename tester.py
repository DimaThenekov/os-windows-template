...
def main():
    ...
    input_data = ["cd build\n./benchmark.exe output_file.txt 1\nexit\n", "cd build\n./benchmark2.exe 10 2\nexit\n"]
    output_data = ["__PATH__$ __PATH__\\build$ Write throughput: __NUM__ MB/s\nExecution time of ./benchmark.exe: __NUM__ seconds\n__PATH__\\build$ ", "__PATH__$ __PATH__\\build$ : __NUM__\nExecution time of ./benchmark2.exe: __NUM__ seconds\n__PATH__\\build$ "]
    ...
    def validate_output(test_number, input_value, output_value):
        ...
        cout = re.sub(r": \d+(\.\d+)?", ": __NUM__", cout)
